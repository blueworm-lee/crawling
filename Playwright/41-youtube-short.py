...
# url = "https://www.youtube.com/shorts/vQxlGqjbV_s"
url = "https://www.youtube.com/shorts/mLFf6KT300k"

# 댓글열기 selector
comment_selectors = [
    # 1. aria-label로 찾기 (가장 안정적)
    "[aria-label*='댓글'][aria-label*='보기']",
    "[aria-label*='댓글']",
    "[aria-label*='Comment']",
    
    # 2. 버튼 구조로 찾기
    "yt-button-shape button[aria-label*='댓글']",
    "button.yt-spec-button-shape-next[aria-label*='댓글']",
    
    # 3. 아이콘으로 찾기
    "yt-button-shape yt-icon",
...
# YouTube Shorts 댓글 selector
reply_list_selectors = [
    "ytd-comment-thread-renderer",
    "#comments ytd-comment-thread-renderer",
    "ytd-engagement-panel-section-list-renderer ytd-comment-thread-renderer"
]
...
def stop_video(page: Page, timeout=5*1000):
    stop_methods = [
        # 1. JavaScript로 직접 정지
            lambda: page.evaluate("document.querySelector('video')?.pause()"),            
            # 2. 비디오 영역 클릭
            lambda: page.click("video"),            
            # 3. Shorts 플레이어 클릭  
            lambda: page.click("ytd-shorts-player"),            
            # 4. 스페이스바
            lambda: page.keyboard.press("Space"),            
            # 5. 비디오 포커스 후 스페이스바
            lambda: (page.click("video"), page.keyboard.press("Space"))
    ]
...
def scroll_to_load_shorts_comments(page: Page, max_comments: int = 100):
    ...
    if not open_shorts_comments(page):
        # 댓글창이 열리지 않으면 일반 스크롤 시도
        print("일반 스크롤로 댓글 찾기 시도...")
    ...
    while True:
        ...
        if current_comments == last_comment_count:
            stale_count += 1
            if stale_count >= 5:  # Shorts는 더 오래 기다림
        ...
        try:
            # 댓글 영역 클릭 후 마우스 휠
            page.click('#contents.style-scope.ytd-item-section-renderer', timeout=1000)
            page.mouse.wheel(0, 1080)
        except Exception as e:
            print(f"마우스 휠 스크롤 실패: {e}")
        
        time.sleep(3)  # Shorts는 로딩이 느릴 수 있어서 더 오래 대기
...
if __name__ == "__main__":
    ...
    print(f"YouTube Shorts URL: {url}")
